...
func Load() (Config, error) {
	...
	if cfg.CancelOddOpsCron == "" {
		return Config{}, errors.New("CANCEL_ODD_OPS_CRON is required")
	}

	_, err = cron.ParseStandard(cfg.CancelOddOpsCron)
	if err != nil {
		return Config{}, fmt.Errorf(
			"CANCEL_ODD_OPS_CRON must be a valid cron expression, got %q: %w",
			cfg.CancelOddOpsCron,
			err,
		)
	}

	cfg.GRPCUnaryTimeout, err = parseNonNegativeDuration(
		"GRPC_UNARY_TIMEOUT",
		valueOrDefault("GRPC_UNARY_TIMEOUT", defaultGRPCUnaryTimeout),
	)
	if err != nil {
		return Config{}, err
	}

	cfg.DBMaxOpenConns, err = parsePositiveInt(
		"DB_MAX_OPEN_CONNS",
		valueOrDefault("DB_MAX_OPEN_CONNS", defaultDBMaxOpenConns),
	)
	if err != nil {
		return Config{}, err
	}

	cfg.DBMaxIdleConns, err = parsePositiveInt(
		"DB_MAX_IDLE_CONNS",
		valueOrDefault("DB_MAX_IDLE_CONNS", defaultDBMaxIdleConns),
	)
	if err != nil {
		return Config{}, err
	}
	...
	cfg.DBConnMaxIdleTime, err = parseNonNegativeDuration(
		"DB_CONN_MAX_IDLE_TIME",
		valueOrDefault("DB_CONN_MAX_IDLE_TIME", defaultDBConnMaxIdleTime),
	)
	if err != nil {
		return Config{}, err
	}

	cfg.DBConnMaxLifetime, err = parseNonNegativeDuration(
		"DB_CONN_MAX_LIFETIME",
		valueOrDefault("DB_CONN_MAX_LIFETIME", defaultDBConnMaxLifetime),
	)
	if err != nil {
		return Config{}, err
	}
